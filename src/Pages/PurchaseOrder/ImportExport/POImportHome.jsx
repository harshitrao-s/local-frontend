...
const POImportHome = () => {
    ...
    const handleExport = async () => {
        ...
        try {
            const res = await apiFetch(
                `${API_BASE}api/purchaseorder/api/export-po?file_format=csv`,
                {
                    method: "GET",
                    responseType: "blob"
                }
            ...
            // 🔹 Fallback timestamp filename
            if (!fileName) {
                const now = new Date();
                const timestamp = now
                    .toISOString()
                    .replace(/[-:]/g, "")
                    .split(".")[0]; // 20260217T160530

                fileName = `po_export_${timestamp}.csv`;
            }
    ...
    const handleDownloadTemplate = async (format) => {
        ...
        try {
            ...
            const res = await apiFetch(`${API_BASE}api/purchaseorder/api/download-template?f_frmt=${format}`, {
            ...
            a.download = `po_import_template.${ext}`;
            document.body.appendChild(a);
            a.click();
            // Memory cleanup
            a.remove();
    ...
    const handleFinalConfirm = async () => {
        ...
        try {
            ...
            if (res.status) {
                toast.success(`${importSummary.valid_count} Records imported successfully!`);
                navigate("/purchaseorder/listing");
            } else {
                showErrorToast(res);
            }
        } catch (err) {
                            ...
                            <div className="d-flex justify-content-between mt-4">
                                ...
                                <Dropdown as={ButtonGroup} size="sm">
                                    ...
                                    <Dropdown.Menu className="shadow border-0 small">
                                        <Dropdown.Item onClick={() => handleDownloadTemplate('csv')}>
                                            <FontAwesomeIcon icon={faFileCsv} className="me-2 text-primary" /> Download as CSV
                                        </Dropdown.Item>
                                        <Dropdown.Item onClick={() => handleDownloadTemplate('xl')}>
